...
export function buildSeating(ctx) {
  ...
  sofa.position.set(-1, FLOOR_Y, 3.5);
  ...
  castAll(sofa, true, true); scene.add(sofa); contactShadow(scene, -1, 3.5, 8);
  ...
  /* 咖啡桌 + 桌上物件 */
  const table = new THREE.Group(); table.position.set(-1, FLOOR_Y, 6.0);
  ...
  // 茶杯 + 托
  const saucer = cyl(0.28, 0.28, 0.04, M.creamShade, 18); saucer.position.set(0.8, 1.31, -0.1); table.add(saucer);
  const cup = cyl(0.16, 0.13, 0.22, new THREE.MeshStandardMaterial({ color: 0xf0ece0, roughness: 0.5 }), 16); cup.position.set(0.8, 1.42, -0.1); table.add(cup);
  // 花瓶 + 几支
  ...
  castAll(table, true, true); scene.add(table); contactShadow(scene, -1, 6.0, 4);
